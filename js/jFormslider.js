...
$.fn.jFormslider = function (options) {
	...
	var navigation_div = '<div  class="mt-4">' + next_button + prev_button + '</div>';
	var divcss =
	{
		"width": width + "px",
		"height": height + "px",
	...
	if (options.next_prev) {
		$(this).find('.li').each(function (index, element) {
			...
			if (index == 0) {
				$(this).append('<div class="mt-4">' + next_button + cancle_button + '</div>');
			} else if (index == lilength - 1) {
				$(this).append('<div class="mt-4" style="display: inline">' + submit_button + prev_button + '</div>');
			} else {
	...
	$.fn.nextSlide = function () {
		...
		current_slide.find('input[required],select[required],input[email],input[questionsNumber]').each(function (index, element) {
			if ($(this).hasAttr('required')) {
				if ($.trim($(this).val()) == '') {
					var msg = $(this).hasAttr('data-msg') ? $(this).attr('data-msg') : 'Please fill this field';
					$('#' + $(this).attr("err-id")).html(msg).show();
					// $(`<div class="text-danger" style="font-size: 12px;">${msg}</div>`).insertAfter(this);
					$(this).focus();
					slidestart = false;
					return false;
				} else {
					$('#' + $(this).attr("err-id")).html(msg).hide();
				}
			}

			if($(this).hasAttr('questionsNumber')) {
				if(parseInt($("#questionNumber").val()) <= 0) {
			...
			if ($(this).hasAttr('salaries')) {
				if (parseInt($("#minSalary").val().replaceAll(",", "")) > parseInt($("#maxSalary").val().replaceAll(",", ""))) {
					var msg = "Maximum salary must be greater than Minimum salary.";
					$('#' + $(this).attr("err-id")).html(msg).show();
					$(this).focus();
					slidestart = false;
					return false;
			...
			if ($(this).hasAttr('email')) {
				if (!emailvalid($.trim($(this).val()))) {
					$('#' + $(this).attr("err-id")).html('Please Enter a valid email').show();
					$(this).focus();
					slidestart = false;
					return false;
				} else {
			...
			if ($(this).is('select')) {
				if ($.trim($(this).val()) == '-1') {
					var msg = $(this).hasAttr('data-msg') ? $(this).attr('data-msg') : 'Please fill this field';
					$('#' + $(this).attr("err-id")).text(msg).show();
					$(this).focus();
					slidestart = false;
					return false;
				} else {
